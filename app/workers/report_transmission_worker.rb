class ReportTransmissionWorker
  ...
  def perform options
    ...
    if @report
      ...
      if report_response.kind_of? Net::HTTPSuccess
        @report.update({ transmitted_at: DateTime.now }) 
        Sidekiq.logger.notice "Report \##{@report.id} has been sent successfully."
      else
        Sidekiq.logger.warn "Report \##{@report.id} is not sent."
      end
    else
      Sidekiq.logger.error "Report \##{options[:id]} does not exist."
    end
  end
